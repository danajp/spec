...
4. If `CMD` in the container configuration is empty,
   1. **IF** the `PACK_PROCESS_TYPE` environment variable is set,
      1. **IF** the value of `PACK_PROCESS_TYPE` corresponds to a process in `<layers>/app.toml`, \
      ...
      2. **IF** the value of `PACK_PROCESS_TYPE` does not correspond to a process in `<layers>/app.toml`, \
         **THEN** launch fails.
